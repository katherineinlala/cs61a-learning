...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    one_digits = player_score % 10
    ten_digits = opponent_score // 10 % 10
    if one_digits == ten_digits:
        return 1
    elif one_digits > ten_digits:
        return 3 * (one_digits - ten_digits)
    else:
        return 3 * (ten_digits - one_digits)
    # END PROBLEM 2
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    max_average = 0
    while i <= 10:
        current_average = make_averaged(roll_dice, times_called)(i, dice)
