...
def interleaved_sum(num: int, f_odd, f_even) -> int:
    ...
    """
    "*** YOUR CODE HERE ***"
    #k is always an odd number. This function returns interleaved sum from k to sum inclusive.
    # def helper(k):
    #     if k == num:
    #         return f_odd(num)
    #     if k + 1 == num:
    #         return f_odd(num - 1) + f_even(num)
    #     return helper(k + 2) + f_even(k + 1) + f_odd(k)

    # return helper(1)
    def helper_odd(a):
        if a == num:
            return f_odd(num)
        return f_odd(a) + helper_even(a + 1)
    def helper_even(b):
        if b == num:
            return f_even(num)
        return f_even(b) + helper_odd(b + 1)
...
def count_dollars(sum_needed: int) -> int:
    ...
    """
    "*** YOUR CODE HERE ***"
    largest_bill = max_bill(sum_needed) #10
    return count_dollars_limit(sum_needed, largest_bill) #count_dollars_limit(15, 10)

def max_bill(sum_needed):
    if sum_needed >= 100:
        return 100
    if sum_needed >= 50:
        return 50
...
def count_dollars_limit(sum_needed, largest_bill):#count_dollars_limit(15, 10)
    if sum_needed == 0:
        return 1
    if sum_needed < 0:
        return 0
    ...
    if largest_bill == 1:
        return 1
    return count_dollars_limit(sum_needed, next_smaller_dollar(largest_bill)) + count_dollars_limit(sum_needed - largest_bill, largest_bill)
