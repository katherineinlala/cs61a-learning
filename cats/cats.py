...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    # selected_p = [p for p in paragraphs if select(p)]
    # if k >= len(selected_p):
    #     return ''
    # return selected_p[k]
    selected = 0
...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def check(paragraph):
        ps = split(lower(remove_punctuation(paragraph)))
        return any(p in subject for p in ps)
    return check
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    if not source:
        return 0.0
    matched = [a for a, b in zip(typed_words, source_words) if a == b]
    return len(matched) / len(typed_words) * 100
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    # min_difference = min(diff_function(typed_word, source_word, limit) for source_word in word_list)
    # if min_difference > limit:
    #     return typed_word
    # for source_word in word_list:
    #     if diff_function(typed_word, source_word, limit) == min_difference:
    #         return source_word
    min_difference = float('inf')
    best_word = typed_word
    for source_word in word_list:
        difference = diff_function(typed_word, source_word, limit)
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == source[i]:
            matched.append(typed[i])
        if typed[i] != source[i]:
            break
    progress = len(matched) / len(source)
...
def fastest_words(match):
    ...
    for j in word_indices:
        ...
        for i in player_indices:
            time = get_all_times(match)[i][j]
            if time < min:
                min = time
                min_player = i
        result[min_player].append(get_all_words(match)[word_indices[j]])
    return result
    # END PROBLEM 10
